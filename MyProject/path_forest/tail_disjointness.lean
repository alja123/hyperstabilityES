...
import MyProject.path_forest.find_ends
...
have hj': j<F1.k-1:= by
  ...
  rw[add_comm]
  exact Nat.lt_of_le_of_lt hj F1k
...
(v: V)
(hv: v ∈ (F.P.get! i).Pa.Wa.support)
(inStarts: v∈  F.S.dropLast)--.droplast
--(hv2: v≠ (F.P.get! i).s)
(hk: F.k=F.S.length)
:
v= (F.P.get! i).s
...
(Fb Fb': Set V)
(k: ℕ )
(F2_avoids_Fb: Path_forest_avoids! iV iSP F Fb k)
(cont: Fb'⊆ Fb)
:
( Path_forest_avoids! iV iSP F Fb' k):= by
sorry
...
have ninS: a ∉ F2.S.dropLast:=by
  apply Path_forest_starts_aux_get
  have hjl: j<F2.k-1:= by
    calc
      j<k:= by exact hj
      _≤ F2.k-1:=by exact F2k
  exact hjl
