...
def MNear_Regular (M: Finset (Subgraph G))(pm:ℕ ):Prop:= ∀ (A: Subgraph G), A ∈ M → near_regular A pm
...
@[ext] structure Clump  (G: SimpleGraph V) where
  Full_Graph: Subgraph G
  p: ℕ
  k: ℕ
  m: ℕ
  κ: ℕ
  ...
  H_In_K: ∀ (A: Subgraph G), A ∈ H → A ≤ Full_Graph
  H_Partition_K: sSup H= Full_Graph
  M_Near_Regular: MNear_Regular M pm -- ∀ (A: Subgraph G), A ∈ M → near_regular A pm
  C_In_K: C≤ Full_Graph
  M_In_C: ∀ (A: Subgraph G), A ∈ M → A ≤ C
  M_Size: M.card = k
  C_Cut_Dense: cut_dense G C (κ^(Nat.factorial (100*k)))
  C_Order: C.verts.toFinset.card ≤  m* 4^(k)
  M_graphs_in_H:  Mgraphs_in_H M H--∀ (A: Subgraph G), A ∈ M→ (∃ (B: Subgraph G), B ∈ H ∧ A ≤ B)
  k_Maximal: ¬ (∃ (M': Finset (Subgraph G)), M'.card = k+1 ∧ (MVertex_Disjoint M')∧ (MNear_Regular M' pm)∧ (Mgraphs_in_H M' H))

def BSet (K: Clump G): Set V
:= {v| v ∈ (K.Full_Graph).verts∧ (((K.Full_Graph.neighborSet v)∩ (sSup K.M: Subgraph G).verts).toFinset.card*K.p ≥ K.m)}
...
theorem near_regular_subgraph {H: Subgraph G}{p pm:ℕ }(hpm: pm>0)(HEdges: p*H.edgeSet.toFinset.card≥ H.verts.toFinset.card^2): ∃ (H': Subgraph G), H' ≤ H ∧ near_regular H' (pm):= by
 sorry
...
∃ (M: Finset (Subgraph G)), M.card = 1 ∧ (MVertex_Disjoint M)∧ (MNear_Regular M pm)∧ (Mgraphs_in_L M L):= by

have h1: ∃ (R: Subgraph G), R ≤ L ∧ near_regular R pm:= by

  exact near_regular_subgraph hpm LEdges

rcases h1 with ⟨R, hR⟩
let M:Finset (Subgraph G):={R}
have MCard: M.card = 1:= by exact rfl
have MNearReg: MNear_Regular M pm:= by
  intros A hA
...
--(hpm: pm=p^2*m)
:∃ (M: Finset (Subgraph G)), M.card = 1 ∧ (MVertex_Disjoint M)∧ (MNear_Regular M pm)∧ (Mgraphs_in_H M H):= by
...
have h1: ∃ (M: Finset (Subgraph G)), M.card = 1 ∧ (MVertex_Disjoint M)∧ (MNear_Regular M pm)∧ (Mgraphs_in_L M L):= by
  exact exists_M_family_in_dense_graph_in_one_subgraph hpm LDense
rcases h1 with ⟨M, hM⟩
...
(MNearReg: MNear_Regular M pm)
(MVertDisj: MVertex_Disjoint M):
L.verts.toFinset.card≥ M.card*pm := by
have h1: ∀ (A: Subgraph G), A ∈ M → A.verts.toFinset.card ≥ pm:= by
  intros A hA
  have hA': near_regular A pm:= by exact MNearReg A hA
...
_≥ ∑ (A∈ M), pm:= by  exact sum_le_sum h1
_=_:= by simp
...
(MNearReg: MNear_Regular M pm)
(MVertDisj: MVertex_Disjoint M):
M.card * pm ≤  ((sSup H: Subgraph G).verts).toFinset.card
:= by
...
(hpm: pm>0)
(Sk: Set ℕ )
(hSk: Sk={ k:ℕ | ∃ (M: Finset (Subgraph G)), (M.card = k) ∧ (MVertex_Disjoint M)∧ (MNear_Regular M pm)∧ (Mgraphs_in_H M H)})
: 1∈ Sk:= by
have h1: ∃ (M: Finset (Subgraph G)), M.card = 1 ∧ (MVertex_Disjoint M)∧ (MNear_Regular M pm)∧ (Mgraphs_in_H M H):= by
  exact exists_M_family_in_cut_dense_family hpm HCutDense HOrderm HNonempty
rcases h1 with ⟨M, hM⟩
rw[hSk]
simp
use M
...
(Sk: Set ℕ )
(hSk: Sk={ k:ℕ | ∃ (M: Finset (Subgraph G)), (M.card = k) ∧ (MVertex_Disjoint M)∧ (MNear_Regular M pm)∧ (Mgraphs_in_H M H)})
:∀ x ∈ Sk, x ≤ ((sSup H: Subgraph G).verts).toFinset.card/pm+1:= by
  intros x hx
  rw[hSk] at hx
  simp at hx
  rcases hx with ⟨M, hM⟩
  ...
  rw[h1.symm]
  have h1: M.card*pm ≤ ((sSup H: Subgraph G).verts).toFinset.card*1:= by calc
   M.card*pm ≤ ((sSup H: Subgraph G).verts).toFinset.card:= by exact M_family_in_H_upper_bound_size hM.2.2.2 hM.2.2.1 hM.2.1
   _= ((sSup H: Subgraph G).verts).toFinset.card*1:= by ring_nf
  calc
  M.card≤ (((sSup H: Subgraph G).verts).toFinset.card/pm+1)*1:= by
    exact nat_div_ge (sSup H:Subgraph G).verts.toFinset.card pm 1 M.card h1 hpmPositive
  _= ((sSup H: Subgraph G).verts).toFinset.card/pm+1:= by ring_nf
...
(hpm: pm>0)
:∃ (k: ℕ),
(∃ (M: Finset (Subgraph G)), (M.card = k) ∧ (MVertex_Disjoint M)∧ (MNear_Regular M pm)∧ (Mgraphs_in_H M H))
∧ ¬(∃ (M: Finset (Subgraph G)), (M.card = k+1) ∧ (MVertex_Disjoint M)∧ (MNear_Regular M pm)∧ (Mgraphs_in_H M H))
∧ k≥ 1
∧ k≤ ((sSup H: Subgraph G).verts).toFinset.card/pm+1
:= by
let Sk: Set ℕ := { k:ℕ | ∃ (M: Finset (Subgraph G)), (M.card = k) ∧ (MVertex_Disjoint M)∧ (MNear_Regular M pm)∧ (Mgraphs_in_H M H)}
let upperbound:ℕ := ((sSup H: Subgraph G).verts).toFinset.card/pm+1
have hbounded: ∀ x ∈ Sk, x ≤  upperbound:= by
  exact fun x a ↦    k_Set_finite hpm      {k | ∃ M, M.card = k ∧ MVertex_Disjoint M ∧ MNear_Regular M pm ∧ Mgraphs_in_H M H} rfl x a
let k:=sSup Sk
have h1inSk: 1∈ Sk:= by
  exact have_1_in_k_Set HCutDense HOrderm HNonempty hpm Sk rfl
...
have h5: ∃ (M: Finset (Subgraph G)), (M.card = k) ∧ (MVertex_Disjoint M)∧ (MNear_Regular M pm)∧ (Mgraphs_in_H M H):= by
  exact h2
have h6: ¬(∃ (M: Finset (Subgraph G)), (M.card = k+1) ∧ (MVertex_Disjoint M)∧ (MNear_Regular M pm)∧ (Mgraphs_in_H M H)):= by
  exact h3
...
{p pm κ m: ℕ}
(hpm: pm>0)
(hLorderUpperBound: L.verts.toFinset.card ≤  m*4)
(hLorderm: L.verts.toFinset.card ≥ m)
(hL: cut_dense G L p):
∃ (X: Clump G), X.Full_Graph=L ∧ X.p=p ∧ X.pm=pm
∧ X.κ=κ ∧ X.m=m ∧ X.H={L}∧ X.C=L
∧ X.k≥ 1∧ X.k≤ L.verts.toFinset.card/pm+1
:= by
...
∧ k≥ 1
∧ k≤ ((sSup H: Subgraph G).verts).toFinset.card/pm+1
:= by exact maximal_k_existence HCutDense HOrderm HNonempty hpm
...
have CCut_dense: cut_dense G C (κ^(Nat.factorial (100*k))):= by
  sorry
have COrder: C.verts.toFinset.card ≤  m* 4^(k):= by calc
  C.verts.toFinset.card = L.verts.toFinset.card:= by rfl
  _≤ m* 4^(1):= by exact hLorderUpperBound
  _≤ m* 4^(k):=by gcongr; exact NeZero.one_le; exact hk.2.2.1
...
let X: Clump G:=⟨L, p, k, m, κ, pm, H, M, C, HEdgeDisj, hM.2.1, HCutDense, HOrderm, HinK, HPartitionK, hM.2.2.1, CinK, MinC, hM.1, CCut_dense, COrder, hM.2.2.2, hk.2.1⟩
use X
...
calc
X.k=k:= by rfl
_≤ ((sSup H: Subgraph G).verts).toFinset.card/pm+1:= by exact hk.2.2.2
_= L.verts.toFinset.card/pm+1:= by rw[HPartitionK.symm]; simp
