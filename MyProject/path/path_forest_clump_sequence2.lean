...
import MyProject.path.find_ends
import MyProject.path.tail_disjointness
...
(prllγ :pr≤ γ)
(ineq: 172 * γ * γ  * k ≤ m / (2 * pr) + 8 * γ  * (2 * k))
(ineq3: γ * 32 + γ * k * 24≤ m/(2*pr))
(ineq4: γ * k * 16 + γ ^ 2 * k * 328 ≤ m/(2*pr))
(γPositive: γ >0)
(αPositive: α >0)
(mggγ: m ≥ 18 * γ)
(mggpr2: m ≥ 18 * pr)
...
rcases ex_HS with ⟨HS, hHS_length, hcutdenseS, hVer_in_HS, hS_in_HS, HSinH, HS_large⟩

rcases ex_HE with ⟨HE, hHE, hcutdenseE, hVer_in_HE, hE_in_HE, HEinH, HE_large⟩
...
have hF1Ex: _:= by
  ...
  calc
    _=
    8 * γ * (Fb ∩ (HS.get! i).verts).toFinset.card + (m / (2 * pr) + 8 * γ * (2 * (k ))):=by
      simp
    _≤
    8 * γ * (Fb).toFinset.card + (m / (2 * pr) + 8 * γ * (2 * (k ))):= by
      gcongr
      simp
    _≤ 8 * γ * (k+4)+ (m / (2 * pr)  + 8 * γ * (2 * (k ))):= by
      gcongr
      dsimp[Fb]
      ...
      calc
        ...
        _= k+4:= by exact hE
    _=γ * 32 + γ * k * 24+m / (2 * pr):= by ring_nf
    _≤ m/pr:= by
      calc
        γ * 32 + γ * k * 24 + (m / (2 * pr) )
        =γ * 32 + γ * k * 24+(m / (2 * pr) ):= by
          ring_nf
        _≤ (m / (2 * pr) )+(m / (2 * pr) ):= by
          gcongr
    ...
    _≤ m/1:= by
      ...
      exact prPositive
    _=m:= by simp
    _≤ (HS.get! i).verts.toFinset.card:=by
      apply HS_large i hi


  --exact γPositive
  --vertex_list_in_graph_list iV iSub Seq.Ver HS (k + 1)
  exact Nat.lt_of_succ_lt kLarge
...
have hF2Ex: _:= by
  apply path_forest_specified_ends_simplified_prefix iV iSub iSP H E Seq.Ver HE  k  m γ pr Fb2 _ _ _ _ _ _ _
  --vertex_list_outside_set iV Seq.Ver Fb (k + 1)
  intro i hi2
  have hi: i< k+1:= by exact Nat.lt_add_right 1 hi2
  dsimp[Fb2]
  ...
  calc
    _
    =
    8 * γ * (Fb2 ∩ (HE.get! i).verts).toFinset.card + (m/(2*pr) + 8 * γ * (2 * (k )))
    :=by
      simp
    _≤
    8 * γ * (Fb2).toFinset.card + (m/(2*pr) + 8 * γ * (2 * (k ))):= by
      gcongr
      simp
    _≤ 8 * γ * (41 * γ * k)+ (m/(2*pr) + 8 * γ * (2 * (k ))):= by
      gcongr
      dsimp[Fb2]
      calc
    ...
    _≤ m/pr:= by
      ring_nf
      calc
        γ * k * 16 + γ ^ 2 * k * 328  + (m / (pr * 2) )
        =γ * k * 16 + γ ^ 2 * k * 328 +(m / (2 * pr) ):= by
          ring_nf
        _≤ (m / (2 * pr) )+(m / (2 * pr) ):= by
          gcongr
    ...
    _≤ m/1:= by
      ...
      exact prPositive
    _=m:= by simp
    _≤ (HE.get! i).verts.toFinset.card:=by
      apply HE_large i hi
