...
import MyProject.newpathforest.find_ends
import MyProject.newpathforest.tail_disjointness
...
have hF1Ex: _:= by
  apply path_forest_specified_ends_simplified_prefix iV iSub iSP H Ver' (S'.rotate 1) HS  (k)  m Fb _ _ _ _ _ _ _
  --vertex_list_outside_set iV Seq.Ver Fb (k + 1)
  intro i hi2
  have hi: i< k+1:= by exact Nat.lt_add_right 1 hi2
  dsimp[Fb]
  sorry--exact id (List.Disjoint.symm hEVer) (Ver_get_in_Ver i hi)
