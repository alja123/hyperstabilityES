...
structure ClumpPathSequence_nolength
 (β : ℕ )(KFam: Finset (Clump G p m κ pr h))  where
  ...
  -- (hlength: Ord.length ≥  h*pr*pr*pr*pr*pr)
...
:=by
have hcard: (BSetPlusM K).toFinset.card ≥  (h*pr*pr*pr ):=by
   calc
    ...
    _≥ h*pr*pr*pr:= by
      ...
      calc
        m≥ 10000*κ^5:= by
          ...
          apply gg2_gg1
          repeat assumption
        _=10000*κ *κ *κ*κ *κ := by
          ring_nf
        _≥ 1*h *h *h*h *h:= by
          gcongr
          simp
          repeat apply gg1_ge; repeat assumption
        _≥ 1*h *pr *pr*pr *pr:= by
          gcongr
          repeat apply gg1_ge; repeat assumption
          --
        _= _:= by
...
have hVex: ∃(VS:Finset V), (VS⊆ (BSetPlusM K).toFinset)∧ (VS.card = (h*pr*pr*pr)):=by
  exact exists_smaller_set (BSetPlusM K).toFinset (h * pr*pr*pr) hcard

have Mcard: K.M.card≥ h*pr*pr*pr+1 :=by
  rw[K.M_Size]

  calc
    K.k≥ pr*pr*pr*pr*h:= by assumption
    _≥ 10000*pr*pr*pr*h:= by
      gcongr
      apply gg1_large2
      exact pPositive
      apply gg2_gg1
    ...
    _≥ 2*pr*pr*pr*h:= by
      gcongr
      simp
    _= h*pr*pr*pr+h*pr*pr*pr:= by
      ring_nf
    _≥ h*pr*pr*pr+1*1*1*1:= by
      gcongr
      repeat assumption
    _= _:= by ring_nf
...
have hMex: ∃(MS:Finset (Subgraph G)), (MS⊆ K.M)∧ (MS.card = (h*pr*pr*pr+1)):=by
  exact exists_smaller_set K.M (h * pr*pr*pr+1) Mcard
...
have LEx: ∃ (L: List (Clump G p m κ pr h)), L.length=(h*pr*pr*pr)+1∧(∀ (i:ℕ ), (i< L.length→ (L.get! i=K)))∧ L.toFinset={K}:=by
  apply define_constant_list_get!
  exact Nat.zero_lt_succ (h * pr*pr*pr)
...
have hi2: i< ML.length:=by
  have h2: ML.length=(h*pr*pr*pr)+1:=by
    rw[MS2.symm]
...
have hlen: VL.length = (h*pr*pr*pr):=by
  rw[hleneq]
  exact VS2
  ...
  _≥ m:=by
    ...
    calc
     ...
     (( (BSetPlusM K).toFinset ∩ (⋃ x, ⋃ (_ : x ∈ KFam ∧ ¬x = K), BSetPlusM x).toFinset).card -
      ...
      _≥
      α *( (BSetPlusM K).toFinset ∩ (⋃ x, ⋃ (_ : x ∈ KFam ∧ ¬x = K), BSetPlusM x).toFinset).card*2 -
      κ *( (⋃ x ∈ Av,(BSetPlusM K).toFinset ∩ BSetPlusM x).toFinset).card / (h*pr*pr*pr)
      :=by
        gcongr
        ...
        repeat assumption
        calc
        _= (2 * α * h * pr*pr*pr) * (⋃ x ∈ Av, (BSetPlusM K).toFinset ∩ BSetPlusM x).toFinset.card := by
          ring_nf
  ...
  calc
    ...
    _≥  h*pr*pr*pr+1:= by
      ...
      calc
        ...
        _=h*pr*pr*pr *α +h*pr*pr*pr *α := by
          ring_nf
        _≥ h*pr*pr*pr *α +1*1*1*1*α := by
          gcongr
          repeat assumption
        _=_:= by ring_nf
...
Knew∈ KFam
∧ (BSetPlusM Knew ∩ BSetPlusM KL).toFinset.card> h*pr*pr*pr
:=by
unfold dense_clump_family at dense_family
have hKint: α * (BSetPlusM KL ∩ ((image BSetPlusM (KFam \ {KL})).toSet).sUnion).toFinset.card ≥ m:=by
  apply dense_family KL
...
by_contra contr
simp at contr
have hunion: (BSetPlusM KL ∩ ((image BSetPlusM (KFam \ {KL})).toSet).sUnion).toFinset.card≤ h*pr*pr*pr*KFam.card:=by
  simp only [coe_image, coe_sdiff, coe_singleton, Set.sUnion_image, mem_coe,
...
have hc1: α * h*pr*pr*pr*(KFam.card)≥ m:=by
  calc
    α * h*pr*pr*pr*(KFam.card)
    =
    α * (h*pr*pr*pr*(KFam.card)):= by
      ring_nf
    _≥
...
Knew∈ KFam
∧ Knew∉Av
∧ (Knew.Gr.verts ∩ KL.Gr.verts).toFinset.card> h*pr*pr*pr
:=by
unfold dense_clump_family at dense_family
...
have hineq: t' ≤ h*pr*pr*pr:=by exact Nat.le_of_succ_le ht
...
:=by
have hex:∃ (S: ClumpPathSequence_nolength_noM iI iV KFam), S.Ord.length=(h*pr*pr*pr)+1∧ S.Ver.length=(h*pr*pr*pr)∧ S.Ord.Nodup:=by
  apply dense_list_implies_path_sequence
  repeat assumption
  exact Nat.le_refl (h * pr*pr*pr)
  repeat assumption
