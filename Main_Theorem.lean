...
-- Theorem: Fix `ε > 0`, and let `C` be sufficiently large compared to `ε`. Let `P` be a path with `d ≥ C` edges. For any graph `G` with `e(G) ≥ ε * d * |G|` having no copies of `P`, it is possible to delete `ε * e(G)` edges to get a graph `H` each of whose connected components has a cover of order ≤ `C * d`.
...
(G_has_no_d_paths : ∀ (u v : V), ∀ (P : K.Walk u v), (P.IsPath ∧ P.toSubgraph ≤ G) → P.length < d) -- This assumption says that `G` has no paths of length `d`. More precisely it reads "for any pair of vertices `u` and `v` in `V`, for any walk `P` from `u` to `v` in `G` which is a path, the length of `P` is less than `d`".
